...
Proof.
  ...
  - inversion H;subst.
    + apply E_IfFalse.
      * simpl in H5. destruct (beval st b);try discriminate. reflexivity.
      * assumption. 
    + apply E_IfTrue.
      * simpl in H5. destruct (beval st b);try discriminate. reflexivity.
      * assumption. 
...
Proof.
  intros b c Beq st st'.
  split; intros H.
  - apply WHILE_true_nonterm with (c:= c) (st:= st) (st':=st') in Beq. 
    apply Beq in H. contradiction.
  - assert (bequiv true true). {
    unfold bequiv in *.
    intros st''. reflexivity.
    } 
    apply WHILE_true_nonterm with (c:= SKIP%imp) (st:= st) (st':=st') in H0. 
...
Proof.
  ...
  -
    assert (Hx : st' =[ x ::= e ]=> (x !-> aeval st' e ; st')). {apply E_Ass. reflexivity. }
    assert (Hy: aeval st' e = st' x). {
  ...
  - 
    ...
    rewrite Hy. rewrite t_update_same.
    apply E_Skip.
Qed.
(** [] *)
...
Proof.
  unfold cequiv.
  intros c1 c1' c2 c2' Hceq1 Hceq2 st st'.
  split;intros H.
  - inversion H;subst.
    apply E_Seq with (st'0).
    + apply (Hceq1 st st'0). assumption.
    + apply (Hceq2 st'0 st'). assumption.
  - inversion H;subst.
    apply E_Seq with (st'0).
    + apply (Hceq1 st st'0). assumption.
    + apply (Hceq2 st'0 st'). assumption.
...
Proof.
  unfold cequiv.
  intros b b' c1 c1' c2 c2' Hbeq Hceq1 Hceq2 st st'.
  split;intros H.
  - inversion H;subst.
    + apply E_IfTrue.
      * rewrite <- Hbeq;assumption.
      * apply (Hceq1 st st');assumption.
    + apply E_IfFalse.
      * rewrite <- Hbeq;assumption.
      * apply (Hceq2 st st');assumption.
  - inversion H;subst.
    + apply E_IfTrue.
      * rewrite Hbeq;assumption.
      * apply (Hceq1 st st');assumption.
...
Proof.
  ...
  - (* WHILE *)
    ...
    destruct (b') eqn:Heqb; try (apply CWhile_congruence;assumption).
...
Lemma optimize_comm: forall a1 a2,
  aequiv ((optimize_0plus_aexp a1)+(optimize_0plus_aexp a2)) (optimize_0plus_aexp(APlus a1 a2)).
Proof.
  intros a1 a2. unfold aequiv.
  intros st. simpl.
...
Theorem optimize_aexp_sound: forall a,
  aequiv a (optimize_0plus_aexp a).
Proof.
  intros a. unfold aequiv.
  intros st.
  induction a; try reflexivity; try (simpl; rewrite IHa1, IHa2; reflexivity).
  simpl. destruct a1 eqn:E; try (simpl; rewrite IHa2; reflexivity); try (rewrite IHa1;rewrite IHa2;reflexivity).
...
Proof.
  ...
  induction H;intros ni st;try reflexivity;try(
    simpl;
    rewrite (IHvar_not_used_in_aexp1 ni st);
    rewrite (IHvar_not_used_in_aexp2 ni st);
    reflexivity
  ).
...
Proof.
  ...
  simpl in H.
  assert (empty_st =[SKIP]=>empty_st) by (apply E_Skip).
  apply H in H0.
...
Proof. 
  unfold pcopy. unfold ptwice. unfold cequiv.
  right.
  intros H.
  assert (empty_st =[(HAVOC X;; HAVOC Y)%imp]=> (X !-> 1;Y!-> 2)). {
    apply E_Seq with (X !-> 1).
    - apply E_Havoc.
    - replace (X !-> 1;Y!-> 2) with (Y !-> 2; X !-> 1).
      apply E_Havoc.
      apply t_update_permute. 
      intros con;discriminate.
  }
  ...
  assert ((Y !-> n; X !-> n) X = (X !-> 1; Y !-> 2) X) by (rewrite H7;reflexivity). 
  unfold t_update in H1. simpl in H1.
  rewrite H1 in H7.
  assert ((Y !-> 1; X !-> 1) Y = (X !-> 1; Y !-> 2) Y) by (rewrite H7;reflexivity). 
...
Proof.
  unfold p1.
  intros st st' HP Heval.
  inversion Heval;subst.
  - simpl in H3.
    assert (st' X = 0). { 
      destruct (st' X =? 0) eqn:E. 
      - apply eqb_eq in E. assumption. 
      - discriminate.
    }
