...
Proof.
  ...
  apply hoare_asgn.
  intros st H. unfold assn_sub, t_update. simpl. omega. Qed.
...
(* because a might contain X, whose value will change
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + unfold assn_sub, bassn, assert_implies, t_update.
...
Proof.
  intros P Q b c HTrue Himpl st st' Heval Pre.
  inversion Heval;subst.
  - apply (HTrue st st').
    assumption. split;assumption.
  - apply Himpl. split;try assumption.
    intros contra. unfold bassn in contra. rewrite H3 in contra. discriminate.
Qed.
...
Proof. 
  ...
  - eapply hoare_consequence_pre. 
    + apply hoare_asgn.
    + unfold assn_sub, assert_implies, bassn, t_update. 
      simpl. intros st [H1 H2]. assumption.
  - unfold assn_sub, assert_implies, bassn, t_update.
    simpl. intros st [H1 H2]. unfold not in H2.
    destruct (st Y =? 0) eqn: E.
    + apply eqb_eq in E. omega.
    + simpl in H2. assert (true = true) by reflexivity.
...
(* FILL IN HERE *)
Theorem hoare_repeat: forall P R b c,
  {{ P }} c {{ R }} ->
  {{ fun st => R st /\ ~ (bassn b st) }} c {{ R }}->
  {{ P }} 
  REPEAT c UNTIL b END 
  {{ fun st => R st /\ bassn b st }}.
Proof.
  intros P Q b c Hass Hinvi st st' Heval HP.
  remember (REPEAT c UNTIL b END) as com eqn:Hcom.
  generalize dependent P.
  induction Heval;try (inversion Hcom);subst; clear Hcom.
  - (*repeat false.*)
    intros P Hass HP.
    apply IHHeval2 with (P := fun st : state => Q st /\ ~ bassn b st). reflexivity. assumption.
    split.
    + apply (Hass st st'). assumption. assumption.
...
(*NOTE: the above proof's key is the generalized P. -HZL *)
...
Theorem assert_assume_differ : exists P b Q,
  ...
  /\ ~ ({{P}} ASSERT b {{Q}}).
Proof.
  exists (fun st => st X = 1).
  exists (X = 2)%imp.
  exists (fun st => st X = 1).
  split.
  - intros st st' H1 H2. inversion H1.
    subst. exists st. split. reflexivity.
    assumption.
  - intros H. unfold hoare_triple in H.
    assert ((X !-> 1)=[ASSERT (X = 2)]=>RError). {
      apply E_AssertFalse. reflexivity.
    }
    apply H in H0.
    + destruct H0 as [st [H2 H3]].
      inversion H2.
    + reflexivity.
Qed.
...
Theorem assert_implies_assume : forall P b Q,
  ...
  -> ({{P}} ASSUME b {{Q}}).
Proof.
  intros P b Q Hhoare st st' Heval Hp.
  inversion Heval;subst.
  exists st.
