...
Proof.
  intros m.
  apply hoare_consequence_post with (fun st => parity (st X) = parity m /\ ~ bassn (2 <= X) st).
  - apply hoare_consequence_pre with (fun st => parity (st X) = parity m).
    + apply hoare_while.
      eapply hoare_consequence_pre.
      apply hoare_asgn.
      unfold assn_sub, assert_implies, bassn;simpl. {
        intros st [H1 H2].
        destruct (st X) eqn:E.
        - discriminate.
        - destruct (n) eqn:F.
          ...
          + subst. rewrite t_update_eq.
            ...
            assert (st X >= 2) by omega.
            apply parity_ge_2 in H. 
            rewrite E in H.
            replace (S (S n0) - 2) with (n0) in H by omega.
            rewrite H. rewrite H1. reflexivity.
      } 
    + intros st H. rewrite H. reflexivity.
  - intros st [HP HBe].
    unfold bassn, not in HBe.
    rewrite <- HP.
    symmetry.
    apply parity_lt_2.
    intros H.
...
Proof.
  ...
  - intros P' Hp.
    ...
    intros st H.
    remember (Hp st (X !-> aeval st (Y + 1); st)) as Q;clear HeqQ.
    simpl in Q. rewrite t_update_eq in Q.
    assert (st Y + 1 <= 5 <-> st Y <= 4) by omega.
    apply H0. apply Q.
...
Proof.
  ...
  - intros P' Hhoare.
    unfold assert_implies, assn_sub.
    intros st Hp.
    apply (Hhoare st (X !-> aeval st a; st)).
    + apply E_Ass. reflexivity.
    + assumption.
Qed.
(** [] *)
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : string),
  ...
  P ->> havoc_pre X Q.
Proof.
  intros P Q X Hhoare st Hp n.
  apply (Hhoare st (X !-> n; st)).
  - apply E_Havoc.
  - assumption.
Qed.
End Himp2.
...
Proof.
  ...
  - destruct (st X) eqn:E.
    ...
    + replace (S n - 1) with (n) by omega.
      rewrite <- H.
      replace (real_fact (S n)) with (S n * real_fact n) by reflexivity.
...
Proof.
  destruct n eqn:E;intros H.
  - omega.
  - replace (Init.Nat.pred (S n0)) with (n0) by omega.
    replace (S n0 + 1) with (S (S n0)) by omega.
    replace (fib (S (S n0))) with (fib (S n0) + fib n0) by reflexivity.
    reflexivity.
Qed.
(** [] *)
...
Definition dfib (n : nat) : decorated
  := 
    ...
    WHILE ~(X = n + 1) DO
      {{ fun st => st Y = fib (st X - 1) 
        /\ st X > 0
        /\ st Z = fib (st X) /\ ~(st X = n + 1) }} 
        ->> 
      {{ fun st => st Z = fib ((st X + 1) - 1) 
        /\ st X + 1 > 0
        /\ st Z + st Y = fib (st X + 1) }}
      T ::= Z
      {{ fun st => st T = fib ((st X + 1) - 1) 
        /\ st X + 1 > 0
        /\ st Z + st Y = fib (st X + 1) }};;
      Z ::= Z + Y
      {{ fun st => st T = fib ((st X + 1) - 1) 
...
Proof.
  intros n;verify.
  - replace (st X + 1 - 1) with (st X) by omega.
    reflexivity.
  - replace (st X -1 ) with (Init.Nat.pred (st X)) by omega.
