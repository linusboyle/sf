...
Proof.
  ...
  intros [M1 M2].
  split.
  - intros H. apply H1 in H. apply M1 in H. apply H.
  - intros H. apply M2 in H. apply H2 in H. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. split.
    ...
    + intros [x [H1 H2]]. apply H2.
  - simpl. split.
    + intros [E1 | E2].
      * exists h. split. { apply E1. } { left. reflexivity. }
      * apply IH in E2. destruct E2 as [u [D1 D2]]. exists u.
        split. {apply D1. } { right. apply D2. }
    + intros [u [L [M|R]]].
      * left. rewrite <- M in L. apply L.
      * right. apply IH. exists u. split.
        { apply L. } { apply R. }
Qed.
...
Proof.
  ...
  - simpl. split.
    + intros [E1|E2].
      * left. left. apply E1.
      * apply IHl in E2. destruct E2.
        { left. right. apply H. }
        { right. apply H. }
    + intros [[H1 | H2] | H3].
      * left. apply H1.
      * assert (In a l \/ In a l') as Q. {
        left. apply H2.
      }
        apply IHl in Q.
        right. apply Q.
...
Proof.
  ...
  - simpl. split.
    + reflexivity.
    + intros H. intros x. apply ex_falso_quodlibet.
  - simpl. split.
    + intros H. split.
      * apply H. left. reflexivity.
      * apply IHl. intros u H'.
        apply H. right. apply H'.
    + intros [E1 E2].
      intros u [H|H'].
      * rewrite H in E1. apply E1.
      * rewrite <- IHl in E2.
        apply E2 in H'. apply H'.
Qed.
(** [] *)
...
Definition combine_odd_even (Podd Peven : nat -> Prop) : nat -> Prop := fun n => if oddb n then Podd n else Peven n.
...
Proof.
  ...
  + unfold combine_odd_even. rewrite E. simpl.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    intros l2. rewrite (IHl1 (x :: l2)). rewrite (IHl1 [x] ).
...
Proof.
  ...
  - reflexivity.
  - unfold tr_rev. unfold tr_rev in IH.
    simpl. rewrite <- IH. apply rev_append_tail. 
Qed.
(** [] *)
...
Proof.
  ...
  - destruct (evenb n) eqn:E.
    + 
      ...
      rewrite <- evenb_S in H. rewrite H.
      destruct IHn as [m IHn].
      exists m. rewrite IHn. reflexivity.
    + 
      ...
      rewrite <- evenb_S in H. rewrite H.
      destruct IHn as [m IHn].
      exists (S m). rewrite IHn. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - unfold not. rewrite <- eqb_eq.
    intros H. rewrite H. intros D. discriminate D.
  - unfold not. intro H. rewrite <- eqb_eq in H.
    destruct (x =? y) eqn:E.
...
Proof.
  ...
  - simpl. destruct l2.
    ...
    + split. { intros D. discriminate D. }
             { intros D. discriminate D. }
  - simpl. destruct l2.
    + split. { intros D. discriminate D. }
             { intros D. discriminate D. }
    + destruct (feq x x0) eqn: E.
      * rewrite IHl1.
        apply pre in E.
        rewrite E. split. { intros P. rewrite P. reflexivity. }
                          { intros P. injection P as goal.
                            apply goal.
                          }
      * split. { intros D. discriminate D. }
               { intros P. injection P as D.
                 apply pre in D.
                 rewrite D in E.
                 discriminate E.
               }
...
Proof.
  ...
  - simpl. split.
    + destruct (f x) eqn:E.
      * simpl. intros H. split. { reflexivity. }
                        { apply IHl in H. apply H. }
      * simpl. intros D. discriminate D.
    + intros [A B]. rewrite A. simpl. apply IHl in B.
      apply B.
Qed.
...
Proof.
  ...
  intros E.
  intros x.
  assert (P x \/ (P x -> False)) as H. { apply EM. } 
  - destruct H as [C1 | C2].
    + apply C1.
    + destruct E. exists x. apply C2.
Qed.
(** [] *)
