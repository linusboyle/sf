...
Theorem SSSSev__even_more : forall n,
  even (S (S (S (S n)))) -> even n.
Proof.
  intros n E.
  ...
  - destruct H as [n' [H1 H2]].
    injection H1 as E.
    rewrite <- E in H2.
    apply ev_inversion in H2. destruct H2.
    + discriminate H.
...
Proof.
  intros n.
  split.
  - intros H. induction H as [| |n' m' EV1 IH1 EV2 IH2].
    + apply ev_0.
    + apply (ev_SS 0 ev_0).
    + apply ev_sum.
      * apply IH1.
      * apply IH2.
  - intros H. induction H as [| n' EV IH].
    + apply even'_0.
    + replace (S(S n')) with (n' + 2).
      * apply even'_sum. { apply IH. } { apply even'_2. }
      * apply plus_comm.
Qed.
(** [] *)
...
Proof.
  ...
  induction N.
  - apply H.
  - simpl in H. apply evSS_ev' in H. apply IHN in H.
    apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - assert (n + n + (m + p) = (n + m) + (n + p)).
    + rewrite plus_assoc. rewrite plus_assoc. rewrite (plus_comm (n+m) n). rewrite plus_assoc. reflexivity.
    + rewrite H0. apply ev_sum. 
      * apply H.
      * apply N.
...
Inductive total_relation : nat -> nat -> Prop :=
  | z n: total_relation n 0
  | s n m (H: total_relation n m): total_relation n (S m).
...
Example empty_relation_ex : ~empty_relation 1 3.
Proof.
  intros H.
  inversion H.
Qed.
...
Lemma le_left: forall m n, S m <= n -> m <= n.
Proof.
  intros m n H.
  induction H.
...
Proof.
  ...
  induction H.
  - apply E.
  - apply IHle. apply le_left. apply E.
Qed.
...
Proof.
  ...
  - destruct n.
    ...
    + simpl. intros P. apply Sn_le_Sm__n_le_m in P. 
      apply IHm in P.
      apply P.
Qed.
...
Lemma plus_0_der : forall m n,
  m + n = 0 -> m = 0  /\ n = 0.
Proof.
  intros m.
  induction m.
  -  simpl. intros n H. split.
    + reflexivity.
    + apply H.
  - simpl. intros n H. rewrite plus_n_Sm in H.
    apply IHm in H. destruct H.
    discriminate H0.
Qed.

Lemma R_plus: forall m n,
  R m n (m + n).
Proof.
  induction m.
  - simpl. induction n.
...
Proof.
  ...
  - intros H. induction H.
    ...
    + simpl in IHR. injection IHR as E. rewrite plus_comm in E.
      simpl in E. injection E as E.
      rewrite plus_comm. apply E.
...
Inductive subseq : list nat -> list nat -> Prop :=
  | nu n: subseq nil n
  | single (n: nat) m (IN: In n m): subseq [n] m
  | con n1 m1 n2 m2 (H1: subseq n1 m1) (H2: subseq n2 m2): subseq (n1++n2) (m1++m2)
.
...
Proof.
  ...
  - apply nu.
  - apply (con [h] [h] t t).
    + apply single. unfold In. left. reflexivity.
    + apply IH.
Qed.
...
Proof.
  intros l1 l2 l3 H.
  replace (l1) with (l1 ++ []).
  - apply (con l1 l2 nil l3).
    + apply H.
    + apply nu.
  - apply app_nil_r.
Qed.

Lemma in_sub_trans: forall n l1 l2,
  In n l1 -> subseq l1 l2 -> In n l2.
Proof.
  intros n l1 l2.
  intros H E.
  induction E.
  ...
  - apply In_app_iff in H.
    destruct H.
    + apply IHE1 in H. apply In_app_iff. left. apply H.
    + apply IHE2 in H. apply In_app_iff. right. apply H.
Qed.

Lemma app_eq_nil : forall (l1 l2: list nat),
  l1 ++ l2 = [] -> l1 = [] /\ l2 = [].
Proof.
  intros l1 l2 H.
  destruct l1.
...
Proof.
  intros T strings re.
  induction strings as [|h t H].
  - simpl. intros D. apply MStar0.
  - intros P. simpl. apply (MStarApp).
    + apply P. simpl. left. reflexivity.
    + apply H. intros s Q. apply P. simpl.
      right. apply Q.
Qed.
(** [] *)
...
Proof.
  ...
  - intros Pre. inversion Pre.
    ...
    + destruct l2.
      * discriminate H1.
      * simpl in H1. injection H1 as H1. rewrite H1 in H0.
Abort.
(** [] *)
...
Proof.
  intros T re. split.
  - induction re.
    + intros [s E]. inversion E.
    + intros [s E].
      reflexivity.
    + intros H. reflexivity.
    + simpl. destruct (re_not_empty re1).
      * simpl. intros [x R]. apply IHre2. 
        inversion R. exists s2.
        apply H3.
      * simpl. intros [x R]. apply IHre1. 
        inversion R. exists s1.
        apply H0.
    + simpl. intros [x R]. inversion R. 
      ...
      * assert (re_not_empty re2 = true). { 
        ...
        rewrite H3. destruct (re_not_empty re1). 
        { reflexivity. }
        { reflexivity. }
    + simpl. intros H. reflexivity.
  - induction re.
    + simpl. intros D. discriminate D.
    + simpl. intros H. exists []. apply MEmpty.
    + simpl. intros H. exists [t]. apply MChar.
    + simpl. intros H. apply andb_true_iff in H. 
      destruct H as [T1 T2].
      apply IHre1 in T1.
      apply IHre2 in T2.
      ...
      * apply T2.
    + simpl. destruct (re_not_empty re1).
      * simpl. intros H. apply IHre1 in H. destruct H as [s1 H].
        exists s1. apply MUnionL. apply H.
      * simpl. intros H. apply IHre2 in H. destruct H as [s2 H].
        exists s2. apply MUnionR. apply H.
    + simpl. intros H. exists nil. apply MStar0. 
Qed.
(** [] *)
...
Proof.
  ...
  - injection Heqre'. intros E. apply IH2 in Heqre'. destruct Heqre' as [ss' [P1 P2]].
    exists (s1::ss'). simpl. rewrite P1.
    split.
    + reflexivity.
    + intros s' [HI1 | HI2]. 
      * rewrite <- E.  rewrite <- HI1. apply Hmatch1.
      * apply P2 in HI2. apply HI2.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. rewrite app_length.
    ...
    destruct H as [R1|R2].
    + apply IH1 in R1. destruct R1 as [s2' [s3' [s4' [P1 [P2 P3]]]]].
      exists s2'. exists s3'. exists (s4' ++ s2).
      split.
      * rewrite P1. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
      * split. { apply P2. } {
        intros m.
        rewrite app_assoc.
        rewrite app_assoc.
        apply MApp.
        {
          rewrite <- app_assoc. apply P3.
        }
        {
          apply Hmatch2.
        }
      }
    + apply IH2 in R2. destruct R2 as [s2' [s3' [s4' [P1 [P2 P3]]]]].
      exists (s1 ++ s2'). exists s3'. exists s4'.
      split.
      * rewrite P1. rewrite <- app_assoc. reflexivity.
      * split. { apply P2. } {
        intros m.
        rewrite <- app_assoc.
        apply MApp.
  ...
  - simpl.
    intros Pre.
    assert (pumping_constant re1 <= length s1) as Q. { omega. }
    apply IH in Q.
    destruct Q as [s2' [s3' [s4' [Q1 [Q2 Q3]]]]].
    exists s2'. exists s3'. exists s4'. split.
    + apply Q1.
    + split.
      * apply Q2.
      * intros P. apply MUnionL. apply Q3.
  - simpl.
    intros Pre.
    assert (pumping_constant re2 <= length s2) as Q. { omega. }
    apply IH in Q.
    destruct Q as [s2' [s3' [s4' [Q1 [Q2 Q3]]]]].
    exists s2'. exists s3'. exists s4'. split.
    + apply Q1.
    + split.
      * apply Q2.
      * intros P. apply MUnionR. apply Q3.
  - simpl. omega. 
  - simpl in *.
    intros H. rewrite app_length in H.
    destruct s1 as [|n1 s1'].
    + simpl. destruct s2 as [|n2 s2'].
      * simpl in *. omega.
      * simpl in *. apply IH2 in H.
        destruct H as [a [b [c [Q1 [Q2 Q3]]]]].
        exists a.
        exists b.
        exists c.
        split. { 
    ...
    + simpl in *.
      ...
      split.
      * reflexivity.
      * split. { intros contra. discriminate contra. }
        {
          simpl. intro m.
          apply star_app.
          - induction m.
            + simpl. apply MStar0.
            + simpl. apply MStarApp with (n1 :: s1') (napp m (n1 :: s1')) re in IHm .
              * apply IHm.
              * apply Hmatch1.
          - apply Hmatch2.
        }
Qed.
    (*
     *assert (1 <= length s2 \/ length s2 = 0). { omega. } 
     *destruct H as [H1 | H2].
     *+ intros pre.
     *  apply IH2 in H1.
     *  destruct H1 as [s2' [s3' [s4' [Q1 [Q2 Q3]]]]].
     *  exists (s1 ++ s2'). exists s3'. exists s4'. split.
     *  * rewrite Q1. rewrite app_assoc. reflexivity.
     *  * split. { apply Q2. }
     *    {
     *      intros m.
     *      rewrite <- app_assoc.
...
Proof.
  ...
  intros H. inversion H.
  - split.
    + intros dummy. reflexivity.
    + intros dummy. apply H0.
  - split.
    + intros D. apply H0 in D. destruct D.
    + intros con. discriminate con. 
Qed.
(** [] *)
...
Proof.
  intros n l.
  induction l as [|h t].
  - simpl. intros H P. destruct P.
  - simpl. destruct (eqbP n h) as [H | H]. 
    + intros D. discriminate D.
    + simpl. intros P [T1 | T2].
      * symmetry in T1. apply H in T1.
        destruct T1.
      * apply IHt in P.
        apply P in T2.
        destruct T2.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | Bs: nostutter nil
  | Dhd (x h: X) (t: list X) (E: h <> x) (H: nostutter (h :: t)):
      nostutter (x :: h :: t)
  | D1 (a : X): nostutter [a]
.
...
Inductive merge {X: Type} : list X -> list X -> list X -> Prop :=
  | MnilL (a: list X): merge a a nil
  | MnilR (a: list X): merge a nil a
  | MhdL (a b c: list X) (h: X) (M: merge a b c): merge (h :: a) (h::b) c
  | MhdR (a b c: list X) (h: X) (M: merge a b c): merge (h :: a) b (h::c)
.

Theorem filter_merge_equiv: forall X (test: X -> bool) (l l1: list X),
  filter test l = l1 <-> exists (l2 : list X), merge l l1 l2 /\
  (forall x, In x l1 -> test x = true) /\ (forall y, In y l2 -> test y = false).
Proof.
  intros X f l l1. split.
  - (*left*)
    generalize dependent l1.
    induction l as [|h t IH].
    + simpl. intros l1 H. rewrite <- H. simpl.
      exists nil.
      split. { apply MnilL. } {
        split.
        - intros x contra. destruct contra.
        - simpl. intros x contra. destruct contra.
      }
    + destruct (f h) eqn:E.
      * simpl. rewrite E. intros l1' H.
        destruct l1' as [|h' t'] eqn:El. {
          discriminate H.
        } {
          injection H as Eqh Eqt.
          apply IH in Eqt.
          destruct Eqt as [l2' [Q1 [Q2 Q3]]].
          rewrite Eqh.
          exists l2'. split.
          - apply MhdL. apply Q1.
          - split. 
            + intros hx. replace (h' :: t') with ([h'] ++ t').
              * intros H. apply In_app_iff in H. destruct H.
                {
                  simpl in H. destruct H.
                  - rewrite <- H. rewrite <- Eqh. apply E.
                  - destruct H.
                }
                {
                  apply Q2 in H.
    ...
    * simpl. rewrite E. intros l1 H. apply IH in H.
      destruct H as [l2' [Q1 [Q2 Q3]]].
      exists (h :: l2').
      split. {
        apply MhdR. apply Q1.
      } {
        split.
        - apply Q2.
        - replace (h :: l2') with ([h] ++ l2').
          + intros y H. apply In_app_iff in H. simpl in H.
            destruct H as [[A|B]|C].
            * rewrite <- A. apply E.
            * destruct B.
            * apply Q3 in C. apply C.
          + reflexivity.
      }
  - 
    intros [l2 [Q1 [Q2 Q3]]]. 
    ...
    + induction a as [|ha ta IHa].
      * reflexivity.
      * assert (f ha = true). { apply Q2. simpl. left. reflexivity. }
        assert (filter f (ha :: ta) = ha :: filter f ta). {
          unfold filter. rewrite H. reflexivity.
        } rewrite H0.
        replace (filter f ta) with (ta). { reflexivity. } {
          symmetry.
          apply IHa.
          intros x Inx. assert (In x (ha :: ta)).
          - replace (ha :: ta) with ([ha] ++ ta).
            + rewrite In_app_iff. right. apply Inx.
            + reflexivity.
          - apply Q2 in H1. apply H1.
        }
    + simpl in *.
      induction a as [|ha ta IHa].
      * reflexivity.
      * assert (f ha = false). { apply Q3. simpl. left. reflexivity. }
        assert (filter f (ha :: ta) = filter f ta). {
          unfold filter. rewrite H. reflexivity.
        } rewrite H0.
        apply IHa.
        intros y Iny. assert (In y (ha :: ta)). {
          - replace (ha :: ta) with ([ha] ++ ta).
            + rewrite In_app_iff. right. apply Iny.
            + reflexivity.
        }
        apply Q3. apply H1.
    + assert (f h = true). { apply Q2. simpl. left. reflexivity. }
      assert (filter f (h :: a) = h :: filter f a). {
        unfold filter. rewrite H. reflexivity.
      } rewrite H0.
      replace (filter f a) with (b). { reflexivity. } {
        symmetry.
        apply IHQ1.
        - intros x Inx. assert (In x (h :: b)).
          + replace (h :: b) with ([h] ++ b).
            * rewrite In_app_iff. right. apply Inx.
            * reflexivity.
          + apply Q2. apply H1.
        - apply Q3.
      }
    + assert (f h = false). { apply Q3. simpl. left. reflexivity. }
      assert (filter f (h :: a) = filter f a). {
        unfold filter. rewrite H. reflexivity.
      } rewrite H0.
      apply IHQ1.
      * intros y Iny. apply Q2. apply Iny. 
      * intros y Iny. assert (In y (h :: c)). {
        replace (h :: c) with ([h] ++ c).
          + rewrite In_app_iff. right. apply Iny.
          + reflexivity.
      }
      apply Q3. apply H1.
...
Inductive pal { X: Type } : list X -> Prop :=
  | Papp (h : X) (l : list X) (H: pal l) : pal (h :: l ++ [h])
  | Pnil : pal nil
.
...
Proof.
  induction l.
  - simpl. apply Pnil.
  - simpl. rewrite app_assoc. apply (Papp x). apply IHl.
Qed.
...
Lemma rev_eq_ht: forall X (l : list X),
  l = rev l /\ l <> nil -> exists h l', l = h :: l' ++ [h].
Proof.
  intros X l.
  induction l.
  - simpl. intros H. destruct H. apply H0 in H. destruct H.
  - simpl.
    intros [P Q].
    destruct (rev l) eqn:E.
    + simpl in P. injection P. intros El. 
Abort.
...
Proof.
  ...
  - simpl in H. destruct H.
  - simpl in H. destruct H. 
    + rewrite H. exists nil. exists l.
      reflexivity.
    + apply IHl in H. destruct H as [l1 [l2 E]].
      exists (x0 :: l1). exists l2. simpl. rewrite <- E.
      reflexivity.
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  |Re2 (x: X): repeats [x;x]
  |Rhd (x: X) (l: list X) (H: repeats l): repeats (x::l)
  |Rta (x: X) (l: list X) (H: repeats l): repeats (l ++ [x])
.
...
Proof.
   ...
   - simpl. intros l2 EXC H. intros contra.
     inversion contra.
   - simpl.
     intros l2 EXC H L.
     apply Rhd. Abort. 
...
Proof.
  ...
  - intros [ s1 [ s2 [ Happ [ Hmat0 Hmat1 ] ] ] ].
    rewrite Happ. apply (MApp s1 _ s2 _ Hmat0 Hmat1).
Qed.
...
Proof.
  ...
  split.
  - intros. inversion H.
    destruct s1 as [|hs1 ts1].
    + simpl. left. split.
      * apply H3.
      * apply H4.
    + simpl in *. injection H1. intros.
      right.
      rewrite H6 in H3. exists ts1. exists s2.
      split.
      * symmetry. apply H5.
      * split. {
        apply H3.
  ...
  - intros. destruct H as [Pre1 | Pre2].
    + destruct Pre1 as [P1 P2].
      apply (MApp nil _ (a :: s0) _ P1 P2).
    + destruct Pre2 as [s1 [s2 [P1 [P2 P3]]]].
      rewrite P1.
      apply (MApp (a :: s1) re0 s2 re1 P2 P3).
Qed.
...
Proof.
  ...
  - remember (Star re) as Re.
    ...
    + discriminate.
    + discriminate.
    + injection HeqRe. intros Eq. 
      (*apply IHexp_match2 in HeqRe.*)
      destruct s1 as [|h t] eqn:E.
      * simpl in *. 
        apply IHexp_match2 in HeqRe. {
          destruct HeqRe as [s1' [s2' goal]].
          exists s1'. exists s2'. apply goal.
        } {
          apply HeqStr.
        }
      * injection HeqStr. intros.
        exists t. exists s2.
        split. {
          symmetry. apply H1.
        } {
          split.
          - rewrite <- H2. rewrite <- Eq. apply H.
          - apply H0.
        }
  - intros. destruct H as [s1 [s2 [A [B C]]]].
    rewrite A.
    apply (MStarApp (a :: s1) s2 _ B C).
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. destruct (match_eps re1).
    + simpl. inversion IHre1. destruct (match_eps re2).
      * apply ReflectT. apply (MApp [] re1 [] re2). { apply H. } 
        { inversion IHre2. apply H0. }
      * apply ReflectF. intros p. inversion p. inversion IHre2.
        assert (s2 = []). {
          destruct s1.
          - apply H1.
          - discriminate H1.
        }
        apply H5. rewrite H6 in H4. apply H4.
    + simpl. apply ReflectF. intros H. inversion H. inversion IHre1.
      apply H5.
      assert (s1 = []). {
        destruct s1.
        - reflexivity.
  ...
  - simpl. destruct (match_eps re1).
    + simpl. apply ReflectT. apply MUnionL. inversion IHre1. apply H.
    + simpl. destruct (match_eps re2).
      * apply ReflectT. apply MUnionR. inversion IHre2. apply H.
      * apply ReflectF. intros H. inversion IHre1. inversion IHre2.
        inversion H.
        {
...
Fixpoint derive (a : ascii) (re : @reg_exp ascii) : @reg_exp ascii :=
  ...
  | EmptySet => EmptySet
  | EmptyStr => EmptySet
  | Char c => if (a =? c)%char then EmptyStr 
                                                 else EmptySet
  | App re1 re2 => if match_eps re1 then Union (App (derive a re1) re2) (derive a re2)
  ...
  - generalize dependent a.
    ...
    + simpl. destruct (match_eps_refl re1).
      * intros s0 a P.
        apply union_disj.
        apply app_ne in P.
        destruct P as [[P1 Q1] | [sa [sb [P2 [Q2 R2]]]]].
        {
          apply IHre2 in Q1. right. apply Q1.
        } 
        {
          apply IHre1 in Q2. left. rewrite P2. apply MApp.
          - apply Q2.
          - apply R2.
        }
      * unfold not in *.
        ...
        destruct P as [[P1 Q1] | [sa [sb [P2 [Q2 R2]]]]].
        {
          apply H in P1. destruct P1.
        }
        {
          rewrite P2. apply MApp.
          - apply IHre1 in Q2. apply Q2.
          - apply R2.
        }
    + simpl. intros s0 a H.
      apply union_disj.
      ...
      destruct H as [R1|R2].
      * apply IHre1 in R1. left. apply R1.
      * apply IHre2 in R2. right. apply R2.
    + simpl. intros s0 a H.
      apply star_ne in H.
      destruct H as [s1 [s2 [P [Q R]]]].
      rewrite P.
      apply MApp.
      * apply IHre in Q. apply Q.
      * apply R.
  - 
    generalize dependent a.
    generalize dependent s0.
    ...
    + simpl in *. intros s0 a H. inversion H. 
    + simpl in *. intros s0 a H. inversion H. 
    + simpl in *. intros s0 a H. destruct ((a =? t)%char) eqn:E.
      * inversion H. apply eqb_eq in E. rewrite E.
        apply MChar.
      * inversion H.
    + simpl in *. destruct (match_eps_refl re1). 
      * intros s0 a Pre. 
        apply union_disj in Pre.
        destruct Pre.
        {
          inversion H0. replace (a :: s1 ++ s2) with ((a :: s1) ++ s2).
          - apply MApp. 
            + apply IHre1 in H4. apply H4.
            + apply H5.
          - reflexivity.
        }
        {
          apply IHre2 in H0.
          apply (MApp nil re1 (a :: s0) re2).
          - apply H.
          - apply H0.
        }
      * unfold not in H.
    ...
    + simpl. intros s0 a H.
      ...
      apply union_disj in H.
      destruct H as [H1 | H2].
      * apply IHre1 in H1. left. apply H1.
      * apply IHre2 in H2. right. apply H2.
    + simpl.
      intros s0 a H.
      inversion H.
      apply ( MStarApp (a :: s1) s2 re).
      * apply IHre in H3. apply H3.
      * apply H4.
Qed.
(** [] *)
...
Fixpoint regex_match (s : string) (re : @reg_exp ascii) : bool :=
  match s with
  | nil => match_eps re
  | cons a s' => regex_match s' (derive a re)
  end.
(** [] *)
...
Proof.
  ...
  - simpl. intros re.
    assert ( x :: s0 =~ re <-> s0 =~ derive x re).
    + apply derive_corr.
    + destruct (regex_match s0 (derive x re)) eqn:E.
      * apply ReflectT. apply H.
        assert (reflect (s0 =~ (derive x re)) (regex_match s0 (derive x re))). {
          apply IHs0.
        }
        rewrite E in H0. inversion H0. apply H1.
      * apply ReflectF. intros P.
        apply H in P.
        assert (reflect (s0 =~ (derive x re)) (regex_match s0 (derive x re))). {
          apply IHs0.
        }
        rewrite E in H0. inversion H0. apply H1 in P.
        destruct P.
Qed.
(** [] *)
