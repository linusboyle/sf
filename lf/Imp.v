...
Proof.
  ...
  - generalize dependent bv. 
    induction b; intros bv H; simpl in *;try(rewrite <- H;constructor;apply aeval_iff_aevalR;reflexivity).
    + rewrite <- H. constructor. apply IHb. reflexivity.
...
Proof.
  ...
  - apply E_Seq with (X !-> 0; Y !-> 1).
    + replace (X !-> 0; Y !-> 1) with (Y !-> 1; X !-> 0).
      * apply E_Ass. reflexivity.
      * apply t_update_permute. intros s. discriminate.
    + replace (X !-> 0; Y !-> 1) with (Y !-> 1; X !-> 0).
      * apply E_Ass. reflexivity.
      * apply t_update_permute. intros s. discriminate.
Qed.
...
Proof.
  ...
  - reflexivity.
  - apply E_Seq with (Y !-> 2; Y !-> 0; X !-> 2);try (apply E_Ass; reflexivity).
  - apply E_WhileTrue with (X !-> 0; Y !-> 3; X !-> 1; Y !-> 2; Y !-> 0; X !-> 2).
    + reflexivity.
    + apply E_Seq with (Y !-> 3; X !-> 1; Y !-> 2; Y !-> 0; X !-> 2); try (apply E_Ass; reflexivity).
    + apply E_WhileFalse.
      reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - injection Heqloopdef. intros P Q.
    rewrite Q in H. simpl in H. discriminate H.
  - injection Heqloopdef. intros P Q.
    apply IHcontra2.
    rewrite P.
    rewrite Q.
...
Proof.
  split.
  - induction c; simpl; intros H; try (subst; constructor).
    + destruct (no_whiles c1) eqn:E.
      * apply IHc1. reflexivity.
      * discriminate.
    + destruct (no_whiles c2) eqn:E.
      * apply IHc2. reflexivity.
      * rewrite andb_comm in H. discriminate.
    + destruct (no_whiles c1) eqn:E.
      * apply IHc1. reflexivity.
      * discriminate.
    + destruct (no_whiles c2) eqn:E.
      ...
      * rewrite andb_comm in H. discriminate.
    + discriminate.
  - intros H. induction H;simpl;try(reflexivity); try (rewrite IHno_whilesR1;rewrite IHno_whilesR2;reflexivity).
Qed.
(** [] *)
...
Proof.
  ...
  induction c;intros st;simpl in *.
  - exists st. apply E_Skip.
  - exists (x !-> aeval st a; st). apply E_Ass. reflexivity.
  - apply andb_true_iff in H. destruct H as [C1 C2].
    apply IHc1 with (st := st) in C1.
    destruct C1 as [s1 E1].
    apply IHc2 with (st := s1) in C2.
    destruct C2 as [s2 E2]. clear IHc1 IHc2.
    exists s2.
    apply E_Seq with s1.
    + apply E1.
    + apply E2.
  - destruct (beval st b) eqn:E. 
    + apply andb_true_iff in H. destruct H as [C1 C2].
      ...
      * apply E1.
    + apply andb_true_iff in H. destruct H as [C1 C2].
      apply IHc2 with (st := st) in C2.
      clear IHc1 IHc2.
      destruct C2 as [s2 E2].
      exists s2.
      apply E_IfFalse.
      * apply E.
      * apply E2.
...
Proof.
  intros st e.
  induction e;try reflexivity;simpl;intros.
...
Fixpoint beval_sc (st: state) (b: bexp): bool :=
  ...
  | BLe a1 a2   => (aeval st a1) <=? (aeval st a2)
  | BNot b1     => negb (beval st b1)
  | BAnd b1 b2  => if beval_sc st b1 then beval_sc st b2
                                  else false
  end.

Theorem beval_equiv: forall st b,
  beval_sc st b = beval st b.
Proof.
  intros st b.
  induction b; simpl; try reflexivity.
  destruct (beval_sc st b1) eqn:E.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_Ass : forall st x n,
      st =[ CAss x n ]=> (x !-> (aeval st n); st) / SContinue
  | E_Break: forall st,
      st =[ CBreak ]=> st / SBreak
  ...
  | E_WhileTrueB: forall b st st' c,
      beval st b = true ->
      st =[ c ]=>st' / SBreak ->
      st =[ WHILE b DO c END ]=> st' / SContinue
  | E_WhileTrueC: forall b st st' st'' c,
      beval st b = true ->
      st =[ c ]=>st' / SContinue ->
      st'=[ WHILE b DO c END ]=> st''/ SContinue ->
      st =[ WHILE b DO c END ]=> st' / SContinue
...
Proof.
  ...
  - subst. rewrite H in H2. discriminate.
...
Proof.
  ...
  split;try(discriminate);try(reflexivity);try(assumption);subst; 
  try(contradiction);
  try(rewrite H in H3; discriminate);
  try(rewrite H in H6; discriminate);
  try(rewrite H in H7; discriminate);
  try(rewrite H in H8; discriminate);
  try(apply IHceval in H5; destruct H5; assumption);
  try(apply IHceval in H6; destruct H6; assumption);
  try(apply IHceval in H7; destruct H7; assumption);
  try(apply IHceval in H8; destruct H8; assumption);
  try(apply IHceval in H9; destruct H9; assumption);
  try(apply IHceval1 in H4; destruct H4; assumption);
  try(apply IHceval1 in H7; destruct H7; assumption);
  try(apply IHceval2 in H5; destruct H5; assumption);
  try(apply IHceval2 in H6; destruct H6; assumption).
Qed.
(** [] *)
End BreakImp.
