...
Fixpoint split {X Y : Type} (l : list (X*Y))
  ...
  match l with 
  | nil => (nil, nil)
  | xy :: remain => ((fst xy) :: (fst (split remain)),
                     (snd xy) :: (snd (split remain)))
  end.
...
Lemma map_tail : forall (X Y: Type) (f : X -> Y) (l : list X) (n : X),
...
Proof.
  ...
  induction l.
  - reflexivity.
  - simpl. rewrite  <- IHl. rewrite -> map_tail.
    reflexivity.
Qed.
(** [] *)
...
(* FILL IN HERE *)
Theorem fold_map_hold : forall (X Y : Type) (f: X -> Y) (l: list X),
...
Definition mult (n m : cnat) : cnat
  := fun (X : Type) (f : X -> X) (x : X) => n X (fun p => m X f p) x.
