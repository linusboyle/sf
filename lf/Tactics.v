...
Proof.
  ...
  - intros m H. simpl in H. 
    ...
    + simpl in H. injection H as P. rewrite <- plus_n_Sm in P. rewrite <- plus_n_Sm in P.
      injection P as Q.
      apply IHn' in Q.
...
Proof.
  ...
  - destruct n as [|n'] eqn: E.
    + simpl. intros P. discriminate P.
    + simpl. intros P. injection P as Q. apply IHl in Q.
      apply Q.
Qed.
(** [] *)
...
(** Prove that [split] and [combine] are inverses in the following
    sense: *)
Lemma app_helper: forall X (l1 l2 : list X) (n: X),
  l1 = l2 -> n::l1 = n::l2.
Proof.
  intros X l1 l2 n.
  intros H.
...
Proof.
  intros X Y l.
  induction l as [|h t].
  - simpl. intros l1 l2 H. injection H as P Q.
    rewrite <- P.
    rewrite <- Q.
    reflexivity.
  - destruct h. 
    ...
    intros l1 l2.
    intros P.
    injection P as e1 e2.
    rewrite <- e1.
    rewrite <- e2.
    simpl. apply app_helper.
...
Proof.
  ...
  - destruct m.
    + destruct p.
      * reflexivity.
      * simpl. intros H Dis. discriminate Dis.
    + destruct p.
      * reflexivity.
      * simpl. intros Dis. discriminate Dis.
  - destruct m.
    + destruct p.
      * simpl. intros Dis. discriminate Dis.
      * simpl. intros Dis. discriminate Dis.
    + destruct p.
      * simpl. intros H Dis. discriminate Dis.
      * simpl. intros H F. apply IHn with (m). {
        apply H.
      } {
        apply F.
      }
...
Proof.
  ...
  - destruct l2.
    + simpl. intros P. discriminate P.
    + intros P. injection P as P.
      apply IHl1 in P.
      assert (combine (x::l1) (x0::l2) = (x, x0) :: combine l1 l2). { reflexivity. }
      rewrite H. simpl.
...
Proof.
  ...
  - destruct (test x0) eqn:E.
    + simpl. rewrite E. intros lf P. injection P as eqx.
      rewrite <- eqx. apply E.
    + simpl. rewrite E. apply IHl.
Qed.
(** [] *)
...
Proof. 
  ...
  - simpl.
    ...
    + simpl. unfold existsb'. simpl. rewrite E.
      simpl. replace (negb (forallb (fun n: X => negb (test n)) l)) with (existsb' test l).
      * apply IHl.
      * reflexivity.
Qed.
